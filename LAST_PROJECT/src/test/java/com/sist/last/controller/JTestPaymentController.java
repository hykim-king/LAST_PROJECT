...
public class JTestPaymentController {
	...
	public void doUpdateTest() throws Exception {
		...
		//2.
		int flag = doInsert(payment01);
		assertThat(flag, is(1)); //flag(actual)과 is(예상) 비교
		
		flag += doInsert(payment02);
		assertThat(flag, is(2)); //flag(actual)과 is(예상) 비교
		
		flag += doInsert(payment03);
		assertThat(flag, is(3)); //flag(actual)과 is(예상) 비교
		
		//3.
		payment01.setOptone(payment01.getOptone()+"99");
		payment01.setOpttwo(payment01.getOpttwo()+"99");
		payment01.setQuantity(payment01.getQuantity()+99);
		payment01.setStatus(payment01.getStatus()+1);
		
		LOG.debug("payment01:"+payment01);
		
		flag = doUpdate(payment01);
		assertThat(flag, is(1));
		
		//3.1 수정 데이터 조회
		Payment changePayment = (Payment) doSelectOne(payment01);
